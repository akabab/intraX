  ...
  if (idAccount.length == 24) {
    if (urlCategory && path) {
      accounts_get().then(function(allAccounts) {
        accounts_get({'_id': idAccount}).then(function(theAccount) {
          category_get().then(function(lstCategories) {
            treeTopic = category_tree_topic({'list': lstCategories, 'root': null});
            idTopic = category_url({'tree': treeTopic, 'path': path});
            topic_get({'categoryId': idTopic}).then(function(topics) {
              topics = topics_uid({'listAccounts': allAccounts, 'listTopics': topics});
              topics = topic_see({'listTopic': topics, 'listSee': theAccount[0].topicSeeNot});
...
/*
** The function returns the new list of topics with boolean information about
** see or not see.
*/

function topic_see(argument) {
  console.log('topic_see');
  var listTopic = argument.listTopic;
  var listSee = argument.listSee;
  var countTopic;
  var countSee;

  for (countSee = listSee.length - 1; countSee >= 0; countSee--) {
    for (countTopic = listTopic.length - 1; countTopic >= 0; countTopic--) {
      if (listSee[countSee] == listTopic[countTopic]._id) {
        listTopic[countTopic].notSee = true;
        break ;
      }
    }
  }
...
** The function returns void and adds a new message from topic' collection
** according to the id from category' collection.
*/
...
function topic_new(argument) {
  ...
  var url = topic_encode({'name': name});
  var date = new Date();
  var data = {'description': name, 'url': url,
              'idAccounts': idAccount, 'dateOfCreation': date};
...
/*
** The function returns a new list of topic with the logins rather than ids.
*/

function topics_uid(argument) {
  var listAccounts = argument.listAccounts;
  var listTopics = argument.listTopics;
  var countAccount;
  var countTopic;

  for (countTopic = listTopics.length - 1; countTopic >= 0; countTopic--)
    for (countAccount = listAccounts.length - 1; countAccount >= 0; countAccount--)
      if (listAccounts[countAccount]._id == listTopics[countTopic].idAccounts) {
        listTopics[countTopic].idAccounts = listAccounts[countAccount].login;
        break ;
      }
  return (listTopics);
}
...
/*
** The function returns the url topic.
*/

function topic_encode(argument) {
